...
# -------------------------
# Prep target genes for codeml using codeml_prep3.py
# -------------------------
def codeml_prep_target(gene, gene_dir, tree, templates_dir):
    description = f"Prep target {gene} for codeml"
    #define input paths
    alt_temp = os.path.join(templates_dir, "alt_template2.txt")
    null_temp = os.path.join(templates_dir, "null_template2.txt")
    branch_temp = os.path.join(templates_dir, "branch_template2.txt")
    M0_temp = os.path.join(templates_dir, "M0_template2.txt")    
...
#Define paths
base = os.getcwd()
data = os.path.join(base, "5data")
result = os.path.join(base, "5results")
